...
export default function Sidebar({ farms, activeFarm, onSelectFarm, activeTab, onSelectTab }) {
  const navItems = [
  ...
  return (
    <nav className="bg-surface-container-lowest flex flex-col h-full border-r border-border-subtle w-[240px] shrink-0 z-30 select-none">
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              ...
              className={`w-full px-3.5 py-2.5 flex items-center gap-3 text-left transition-all border-l-2 text-xs font-bold uppercase tracking-wider ${
                isActive
      ...
      <div className="p-4 border-t border-border-subtle bg-surface shrink-0 flex items-center justify-between font-mono-data text-[10px] text-secondary">
        ...
        <span className="font-bold">v2.0 PRO</span>
      </div>
    </nav>
  );
}
