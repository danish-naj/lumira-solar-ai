...
export default function MaintenanceHub({ farm, onRefreshFarm, onNavigateTab, onSelectModule }) {
  ...
  const filteredOrders = workOrders.filter((order) => {
    ...
    if (inverterFilter !== "ALL" && order.inverter_id !== inverterFilter) return false;
    if (searchQuery.trim()) {
      const q = searchQuery.toLowerCase();
      const matchId = order.id?.toLowerCase().includes(q);
      const matchMod = order.module_id?.toLowerCase().includes(q);
      const matchInv = order.inverter_id?.toLowerCase().includes(q);
      const matchDef = order.defect_type?.toLowerCase().includes(q);
      const matchTech = order.assigned_technician?.name?.toLowerCase().includes(q);
      if (!matchId && !matchMod && !matchInv && !matchDef && !matchTech) return false;
    }
    return true;
  });
"""
